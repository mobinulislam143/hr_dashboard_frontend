...
import { useAddRepMutation } from '@/store/api/repApi';
import { BUSINESS_LABELS, ROLE_LABELS, Business, RepRole } from '@/types';
...
export function AddRepModal({ onClose, onSuccess }: Props) {
  const [addRep, { isLoading }] = useAddRepMutation();

  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    resolver: zodResolver(schema),
    defaultValues: { hireDate: new Date().toISOString().split('T')[0] },
  });

  const onSubmit = async (data: FormData) => {
    try {
      await addRep({ ...data, recruitingSource: 'OTHER' } as any).unwrap();
      toast.success('Rep added successfully');
      onSuccess();
    } catch (err: any) {
      toast.error(err?.data?.message || 'Failed to add rep');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-card border border-border rounded-2xl shadow-2xl animate-fade-in max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-border sticky top-0 bg-card rounded-t-2xl">
          <div>
            <h2 className="font-semibold text-foreground text-lg">Add Active Rep</h2>
            <p className="text-muted-foreground text-xs mt-0.5">Manually add a rep directly to the active roster</p>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input {...register('hireDate')} type="date" className="form-input" />
          </div>
